...
(check-expect (unique-award-names P8)
              (list "SIGGRAPH" "Sloan" "VGTC" "ACM" "VisAcademy"
                    "Google" "Microsoft"))
(check-expect (unique-award-names P9) (list
                                       "Turing"
                                       "Oscar"
                                       "Hellman"
                                       "SIGGRAPH"
...
(@template-origin use-abstract-fn fn-composition (listof String))

(define (unique-award-names prof)
  (local [(define (c1 nam cit awd stu)
            (append awd stu))
          
          (define (remove-repeats los)
            (cond [(empty? los) empty]
                  [else
                   (if (member? (first los) (rest los))
                       (remove-repeats (rest los))
                       (cons (first los) (remove-repeats (rest los))))]))]
    
    (remove-repeats (fold-prof c1 cons append empty empty prof))))
#;
(@signature (String Natural Y Z -> X)
            (String Y -> Y)
            (X Z -> Z)
