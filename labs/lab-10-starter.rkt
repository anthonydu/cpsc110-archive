...
;; takes a PrefixTree and returns the number of odd-length words in it
(check-expect (count-odds PT0) 1)
(check-expect (count-odds PTT) 4)

(@template-origin PrefixTree (listof PrefixTree) encapsulated accumulator)
#;
(define (count-odds pt)
  (local [(define (fn-for-pt pt l pt-wl l-wl count)
            (if (and (pt-f? pt) (odd? (add1 l)))
                (fn-for-lopt (append (pt-subs pt) pt-wl)
                             (append (make-list (length (pt-subs pt)) (add1 l))
                                     l-wl)
                             (add1 count))
                (fn-for-lopt (append (pt-subs pt) pt-wl)
                             (append (make-list (length (pt-subs pt)) (add1 l))
          ...
          (define (fn-for-lopt pt-wl l-wl count)
            (cond [(empty? pt-wl) count]
                  [else
                   (fn-for-pt (first pt-wl)
                              (first l-wl)
                              (rest  pt-wl)
                              (rest  l-wl)
                              count)]))]
    
    (fn-for-pt pt 0 empty empty 0)))
...
;; encode a string in the tree into a list of binary digits, false if not found
...
(define (encode ht s)
  (local [(define (encode ht rsf)
            (cond [(string? ht) (if (string=? ht s) rsf false)]
                  [else
                   (local [(define try0 (encode (htree-zero ht)
                                                (append rsf (list 0))))
                           (define try1 (encode (htree-one ht)
                                                (append rsf (list 1))))]
                     (cond [(not (false? try0)) try0]
                           [(not (false? try1)) try1]
                           [else false]))]))]
