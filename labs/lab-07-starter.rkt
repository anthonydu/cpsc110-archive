...
; /HTC/lab/lab-07-yes-no-tree.png")
...
(define YNT1 (make-yntree "Does it gobble?"
                          ...
                          (make-yntree "Does it say 'Nevermore'?"
                                       "Raven"
                                       "Eagle")))
(define YNT2 (make-yntree "Does it gobble?"
                          (make-yntree "Does it say 'Nevermore'?"
                                       "Raven"
                                       "Eagle")
                          (make-yntree "Does it say 'Nevermore'?"
...
(check-expect (contains-n-path? YNT1 0) false)
(check-expect (contains-n-path? YNT2 1) false)
(check-expect (contains-n-path? YNT2 2) true)
(check-expect (contains-n-path? YNT2 3) false)
...
;;                           0        (add1 Natural)                
;; 
;; y String                  [1](and (string? ynt) (= n 0))
;; n
;; t (make-yntree String     [1]^     [2](or (contains-n-path? (yntree-y ynt)
;;                YesNoTree                                    (sub1 n))
 ...
 (define (contains-n-path? ynt n)
   (cond
     ...
     [else 
      (... n
           (yntree-s ynt)
           (contains-n-path? (yntree-y ynt))
           (contains-n-path? (yntree-n ynt)))])))
...
(check-expect (set-matches (list "Anthony") empty) empty)
(check-expect (set-matches LOPL1 LOPL2)
              (list (make-match "Eugenie"    "Maria")
                    (make-match "Gabriela"   "Nadia")
                    (make-match "Sharon"     "Elena")
                    (make-match "Aleksandra" "Anastasia")))
(check-expect (set-matches (cons "Anthony" LOPL1) LOPL2)
              (list (make-match "Anthony"    "Maria")
                    (make-match "Eugenie"    "Nadia")
                    (make-match "Gabriela"   "Elena")
                    (make-match "Sharon"     "Anastasia")
                    (make-match "Aleksandra" "Svetlana")))

; (define (set-matches lopl1 lopl2) empty) ; stub

;; CROSS PRODUCT OF TYPE COMMENTS TABLE
;;
;;                                    lopl1
;;                              empty       (cons Player ListOfPlayer)
;; 
;; l empty                      [1]empty
;; o
;; p (cons Player ListOfPlayer) [1]^        [2](cons (make-match (first lopl1)
;; l                                                             (first lopl2))
;; 2                                                 (set-matches (rest lopl1)
;;                                                                (rest lopl2)))

(@template-origin 2-one-of)

(define (set-matches lopl1 lopl2)
  (cond [(or (empty? lopl1) (empty? lopl2)) empty]
        [else 
         (cons (make-match (first lopl1) (first lopl2))
               (set-matches (rest lopl1) (rest lopl2)))]))
