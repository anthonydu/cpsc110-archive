...
(check-expect (make-row 3 COOKIES) (beside COOKIES COOKIES COOKIES))

; (define (make-row n i) empty-image) ; stub
...
;; swap bubble and solid if they are in the wrong order
(check-expect (swap "solid" empty) (cons "solid" empty))
(check-expect (swap "solid" (list "bubble")) (list "bubble" "solid"))
(check-expect (swap "bubble" (list "solid")) (list "bubble" "solid"))
...
(define (swap b lob)
  (cond [(empty? lob) (cons b lob)]
        [else
         (if (string=? (first lob) "bubble")
             (append (list (first lob) b) (rest lob))
             (cons b lob))]))
