...
;; produce the shortest path if maze is solvable, false otherwise
...
(@template-origin genrec try-catch arb-tree accumulator)
  ...
  (local [(define R (sqrt (length m)))
          ...
          (define (solve/lop lop path)
            (cond [(empty? lop) false]
                  [else
                   (choose-shorter-path (solve/p (first lop) path)
                                        (solve/lop (rest lop) path))]))
          
          (define (choose-shorter-path first shortest-of-rest)
            (cond [(false? first) shortest-of-rest]
                  [(false? shortest-of-rest) first]
                  [(< (length first) (length shortest-of-rest)) first]
                  [else shortest-of-rest]))
