...
(define (find-path t n)
  ;; path is (listof String): names of parent, grandparent. trees
  ;;                          to here (built along recursive calls)
  (local [(define (fn-for-t t path)
            (local [(define name (node-name t))  ;unpack the fields
                    (define subs (node-subs t))] ;for convenience
              
              (if (string=? name n)
                  ; if n is found append n to path
                  (append path (list name)) ; exploit using path
                  ; preserve the inv update path
            ...
            (cond [(empty? lot) false]
                  [else
                   (local [(define try (fn-for-t (first lot) path))]
                     (if (not (false? try))
                         try
                         ; preserve the inv nothing changes
