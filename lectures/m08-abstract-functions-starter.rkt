...
;; produce a list of only those elements of the list for which p is true
...
(define (filter2 p lon) ; predicate
...
(define (all-greater-than n lon)
  (local [(define (>n? x) (> x n))] ; closure, wraps n inside a function
    (filter2 >n? lon)))
...
(check-expect (foldr2 + 0 (list)) 0)
(check-expect (foldr2 + 0 (list 1 -2 3 -4)) -2)
(check-expect (foldr2 - 0 (list 1 -2 3 -4)) 10)

(@template-origin (listof X))
(define (foldr2 c b lox) ; function, base case
  (cond [(empty? lox) b]
        [else
         (c (first lox)
            (foldr2 c b (rest lox)))]))
