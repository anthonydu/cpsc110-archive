...
;; produce shortest path if maze is solvable, false otherwise
...
(define (solve m)
  (local [(define R (sqrt (length m)))
          ...
          (define (solve/p p path p-wl path-wl rsf)
            (cond [(solved? p) (solve/lop p-wl path-wl
                                          ; keep traverse, update rsf
                                          (if (and (> (length path)
                                                      (length rsf))
                                                   (not (empty? rsf)))
                                              rsf
                                              (append path (list p))))]
                  [(member p path) (solve/lop p-wl path-wl rsf)] ; keep traverse
                  [else
