...
(check-expect (arrange-images (cons I2 (cons I1 empty)))
              (beside I1 I2 empty-image))
; no need to check base case for function composition
...
; insert each item into a sorted list of the rest of the items
        ...
        [else
         (if (larger? img (first loi))
             (cons (first loi)
                   (insert img
                           (rest loi)))
             ; keep swapping (move img to the right of the list)
             ; until it's no longer larger than the next item
             (cons img loi))]))
; loi is sorted so there's no need to worry about it

(@htdf larger?)
(@signature Image Image -> Boolean)
;; produce true if img1 is larger than img2  (by area)
(check-expect (larger? (circle 35 "solid" "red")
                       (rectangle 30 40 "solid" "red")) true)
(check-expect (larger? (rectangle 30 40 "solid" "red")
                       (circle 35 "solid" "red")) false)
(check-expect (larger? (rectangle 30 40 "solid" "red")
                       (rectangle 60 20 "solid" "red")) false)

; (define (larger? img1 img2) true) ; stub

(@template-origin Image)
(@template (define (larger? img1 img2) (... img1 img2)))

(define (larger? img1 img2) (> (* (image-width img1)
                                  (image-height img1))
                               (* (image-width img2)
                                  (image-height img2))))
