...
(define (bst? bt0) 
  (local [(define (bst? bt lower upper)
            (cond [(false? bt) true] ; reached the end of the tree
                  [else
                   (and (< lower (node-k bt) upper)               ; exploit
                        ; current node between lower and upper
                        (bst? (node-l bt) lower (node-k bt))      ; preserve
                        ; current node is the upper bound for left side
                        (bst? (node-r bt) (node-k bt) upper)      ; preserve
                        ; current node is the lower bound for right side
                        )]))] 
