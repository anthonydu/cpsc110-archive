...
(@htdf fold-course)
(@signature (Natural Natural Y -> X) (X Y -> Y) Y Course -> X) ; why is b1 Y?
;; 
(check-expect (fold-course make-course cons empty C110) C110)
(check-expect (fold-course add1! + 0 C110) 20)

(define (add1! a b c) (add1 c))

; b1 is the base case result for fn-for-loc
; Course doesn't have a base case
