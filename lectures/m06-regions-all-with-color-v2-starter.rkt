...
; region single different color
(check-expect (all-with-color--region "blue" S1) '())
; region at least 2 long
(check-expect (all-with-color--lor "red" LOR123) (list S1))
; region group resulting in both groups and singles
(check-expect
...
(define (all-with-color--region c r)
  (cond [(single? r)
         (if (string=? c (single-color r))
             (list r)
             empty)]
        [else
         (if (string=? c (group-color r))
