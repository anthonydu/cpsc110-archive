...
;; produce (listof Pos) if maze is solvable, false otherwise
;; CONSTRAINT maze has a true at least in the upper left
(check-expect (solve M1) (list
                          (make-pos 0 0)
                          (make-pos 0 1)
                          (make-pos 1 1)
                          (make-pos 1 2)
...
(define (solve m)
  (local [(define R (sqrt (length m)))	  
          ...
          (define (solve/p p path)
            (cond [(solved? p)     (append path (list p))]
                  [(member p path) false]
                  [else
                   (solve/lop (next-ps p)
                              (append path (list p)))]))
