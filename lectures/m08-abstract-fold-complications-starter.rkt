...
(define (courses-w-credits c n)
  (local [(define (courses-w-credits--course c)
            (local [(define rmr (courses-w-credits--loc (course-dependents c)))]
...
; (define (find-course c n) c) ; stub

(@template-origin Course (listof Course) encapsulated try-catch)

(define (find-course c n)
  (local [(define (find-course--course c)
            (local [(define rmr (find-course--loc (course-dependents c)))]
              (if (= n (course-number c))
                  (course-credits c)
                  rmr)))
