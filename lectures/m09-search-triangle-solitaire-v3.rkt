  ...
  (local [(define (solve-bd bd)
            (if (solved? bd)
                ...
                (local [(define try (solve-lobd (next-boards bd)))]
                  ; solve the list of possible next boards
                  ; (based on possible/valid moves)
                  (if (not (false? try))
                      (cons bd try)
                      ; cons current bd to the result of the mutual recursion
                      false))))
...
(define (next-boards bd)
  (local [(define (do-jump j)
          ...
          (define (valid? j)
            (and (get-cell bd (jump-from j))
                 (get-cell bd (jump-over j))
                 (not (get-cell bd (jump-to j)))))]
    ; return a list of boards after valid jumps
