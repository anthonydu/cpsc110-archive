...
;;   - making a copy before you start
;;   - and renaming the path accumulator to ???
...
(define (find-path t n)
  ;; path is (listof String): names of parent, grandparent... trees to here
  ;;                          (builds along recursive  calls)
  ;; p-wl is (listof (listof String)): worklist of paths
  ;;                                   corresponding to trees in t-wl
  (local [(define (fn-for-t t p t-wl p-wl)
            (local [(define name (node-name t))
                    (define subs (node-subs t))
                    (define newp (append p (list name)))]
              (if (string=? name n)
                  newp ; return the new path updated with the current node
                  (fn-for-lot (append subs t-wl) ; add subs to t-wl
                              ; add subs number of new paths to p-wl
                              ; (this way paths in p-wl matchs trees in t-wl)
                              (append (make-list (length subs) newp) p-wl)))))
          
          (define (fn-for-lot t-wl p-wl)
            (cond [(empty? t-wl) false]
                  [else
                   (fn-for-t (first t-wl) ; go to the next tree in the worklist
                             (first p-wl) ; go to the next path in the worklist
                             (rest  t-wl) ; the new worklist is (rest t-wl)
                             (rest  p-wl) ; the new worklist is (rest p-wl)
                             )]))]
